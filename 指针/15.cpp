...
void avsco(float *pscore,float *paver)                        //求每个学生的平均成绩
{
	int i,j;
	float sum,average;
	for(i=0;i<4;i++)
...
void avcour1(char(*pcourse)[10],float *pscore)               //第1课程的平均成绩
{
	int i;
	float sum,average1;
	sum=0.0;
...
void fail2(char course[5][10],int num[],float *pscore,float aver[4])  //找两门课不及格的学生
{
	int i,j,k,label;
...
void  good(char course[5][10],int num[4],float *pscore,float aver[4])  //找成绩优秀学生（各门85分以上或平均90以上）
{
	int i,j,k,n;
